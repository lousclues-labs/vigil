...
v1.2.4. The v1.2.4 sync incorporates the framework fixes discovered
during vigil's first real adoption plus the GitHub-rejected
`actions/cache` v4.0.2 pin replacement and the fpm stdout-capture
follow-up, so `pkg-framework verify` is clean again: no vendored
drift remains.

### Changed

- **Packaging migrated to `pkg-framework` v1.2.4.** Vigil now vendors
  ...
  `.github/workflows/pkg-build.yml` are preserved at `.archive/` for the
  duration of the migration; they can be removed once one real release
  ships through the framework cleanly.
- **`pkg-build.yml` workflow now matches the framework's vendored
  template.** Same matrix as before (`ubuntu-24.04`, `debian-12`,
  `fedora-40`), same gates (build, install-test, layout-check, manifest
  schema check, reproducibility double-build on PRs). All vigil-specific
  assertions from the pre-framework workflow are re-encoded as
  `project_*` hooks in `pkg/project.sh`.
- **Hermetic cargo build restored through the framework knob.** Vigil
  sets `PKG_CARGO_OFFLINE=1`, which makes pkg-framework run
  `cargo fetch --locked` followed by `cargo build --frozen --offline`.
  This preserves the pre-framework property that the compile step
  cannot touch the network.

### Added

- **`pkg/project.sh` declares everything vigil-specific:**
  `PKG_NAME=vigil-baseline`, `PKG_PREFIX=VIGIL` (kept as `VIGIL`, not
  `VIGIL_BASELINE`, so the `lousclues-pkg` orchestrator's existing
  `VIGIL_MANIFEST_COMMIT` env var keeps feeding the manifest sidecar),
  `PKG_BINARIES=(vigil vigild)`, `PKG_SYSTEMD_UNITS=(vigild.service
  vigil-scan.service vigil-scan.timer)`,
  `PKG_DEB_CONFIG_FILES=(/etc/vigil/vigil.toml.example)`,
  `PKG_LAYOUT_CHECKS` for the 14 canonical installed paths,
  deb/rpm runtime deps, and the extra build deps required by
  openssl-sys and the dnf plugin staging path.
- **Seven `project_*` hooks** preserve every behavior of the
  pre-framework build:

  | Hook | What it does |
  |---|---|
  | `project_pre_install_deb_deps` | Runs `scripts/fix-debian-deps.sh` for the libsystemd0 version-skew remediation. |
  | `project_post_build` | Generates bash/zsh/fish shell completions and three gzipped man pages (`vigil.1`, `vigil.toml.5`, `vigild.8`) from the freshly built `vigil` binary into `$STAGE/_generated/`. |
  | `project_stage_extra` | Copies `_generated/` into the install root, stages `/etc/vigil/vigil.toml.example` with an autogenerated header, and stages `hooks/apt/*` (deb only) plus `hooks/dnf/*` (rpm only). Pins all mtimes to `SOURCE_DATE_EPOCH`. |
  | `project_postinst_body` | `setcap cap_sys_admin,cap_dac_read_search+ep /usr/bin/vigild` with a `--help` self-test and a revert path for minimal container images that break secure-execution after file caps. |
  | `project_validate_stage_extra` | Asserts the apt hook cycle ships as a complete triple (`99vigil` + `apt-pre.sh` + `apt-post.sh`) or not at all; same for the dnf plugin pair; rejects systemd units that reference dev-only paths (`/usr/local/bin/vigild`, `target/debug`, `target/release`). |
  | `project_fpm_rpm_extra_args` | Passes three rpmbuild macros required for sha256-reproducible rpms across rebuilds: `use_source_date_epoch_as_buildtime`, `clamp_mtime_to_source_date_epoch`, `_buildhost reproducible.vigil-baseline.local`. |
  | `project_install_layout_check_extra` | Five vigil-specific post-install gates: vigild has `cap_sys_admin` (file caps OR systemd ambient fallback), apt hook cycle integrity, dnf plugin pair integrity, `systemd-analyze verify` of `vigild.service`, `vigil --version` smoke within 10s. |
...
now live upstream in pkg-framework v1.2.4 and are consumed by sync, not
carried as local vigil patches:

1. **CLI symlink resolution.** `~/.local/bin/pkg-framework version` and
  `doctor` now resolve the real framework home without requiring
  `PKG_FRAMEWORK_HOME`.
2. **Actions container shell.** The vendored workflow sets
  `defaults.run.shell: bash`, so `source pkg/project.sh` works in
  `ubuntu`, `debian`, and `fedora` container jobs.
3. **Multi-line `PKG_DESCRIPTION`.** pkg-framework now builds fpm args
  as arrays rather than newline-delimited strings, so the restored
  two-paragraph vigil description is passed as one `--description`
  argument.
4. **Artifact path capture.** `_pkg_fpm_deb` and `_pkg_fpm_rpm` no longer
  pollute the stdout channel used to return the artifact path.
5. **Artifact validation listing.** `_pkg_validate_artifact` no longer
  SIGPIPEs on packages with more than 20 installed paths.
6. **Slim-image docs/man handling.** Install-test clears dpkg/dnf
  doc-stripping settings before installing the built package, so
  README, changelog, copyright, and man pages actually land in the
  test container.
7. **Debian copyright.** Framework-generated debian/copyright now points
  GPL-3.0-only packages at `/usr/share/common-licenses/GPL-3`; vigil no
...
- **Artifact filenames still change.** Pre-framework:
  `vigil_<v>_amd64-<distro>.deb` with the distro embedded so
  noble/jammy/bookworm could coexist in one OUTDIR. Framework default:
  ...
  v1.2.4 includes optional suffix knobs for consumers that need the old
...
- **No vendored drift remains.** After `pkg-framework sync --bump` to
  v1.2.4, `pkg-framework verify` reports every vendored file clean and
  `FRAMEWORK_VERSION=1.2.4` matches `pkg/lib/VERSION`.
