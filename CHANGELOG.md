...
Packaging-only release. No source changes, no behavior changes
in either binary. The release migrates vigil's deb/rpm
production from a hand-rolled 747-line `pkg/build.sh` to
[pkg-framework](https://github.com/lousclues-labs/pkg-integration)
v1.2.1 and lands six framework workarounds discovered by
exercising the framework end-to-end. Each one is filed
upstream so the next consumer doesn't relearn them.

### Changed

- **Packaging migrated to `pkg-framework` v1.2.1.** Vigil now
  vendors the framework at `pkg/lib/` and declares its
  packaging contract through `pkg/project.sh` (manifest
  fields + `project_*` hooks). The 747-line pre-framework
  `pkg/build.sh` and the 748-line pre-framework
  `.github/workflows/pkg-build.yml` are preserved at
  `.archive/` for the duration of the migration; they can be
  removed once one real release ships through the framework
  cleanly.
- **`pkg-build.yml` workflow now matches the framework's
  vendored template.** Same matrix as before
  (`ubuntu-24.04`, `debian-12`, `fedora-40`), same gates
  (build, install-test, layout-check, manifest schema check,
  reproducibility double-build on PRs). All vigil-specific
  assertions from the pre-framework workflow re-encoded as
  `project_*` hooks in `pkg/project.sh`.

### Added

- **`pkg/project.sh` (487 lines)** declares everything
  vigil-specific:
  - `PKG_NAME=vigil-baseline`, `PKG_PREFIX=VIGIL` (kept as
    `VIGIL`, not `VIGIL_BASELINE`, so the `lousclues-pkg`
    orchestrator's existing `VIGIL_MANIFEST_COMMIT` env var
    keeps feeding the manifest sidecar).
  - `PKG_BINARIES=(vigil vigild)`,
    `PKG_SYSTEMD_UNITS=(vigild.service vigil-scan.service
    vigil-scan.timer)`,
    `PKG_DEB_CONFIG_FILES=(/etc/vigil/vigil.toml.example)`.
  - `PKG_LAYOUT_CHECKS` covering the 14 canonical paths
    (binaries, config example, three man pages, three shell
    completions, two doc files, three systemd units).
  - `PKG_DEB_DEPENDS=(libc6 "libssl3 | libssl1.1" libcap2-bin
    procps)`, `PKG_RPM_REQUIRES=(glibc openssl-libs libcap)`,
...
- **Seven `project_*` hooks** preserve every behavior of the
  ...
  | `project_stage_extra` | Copies `_generated/` into the install root via tar pipe, stages `/etc/vigil/vigil.toml.example` with an autogenerated header, stages `hooks/apt/*` (deb only) and `hooks/dnf/*` (rpm only), and overwrites the framework's truncated debian/copyright with the full machine-readable GPL-3 form. Pins all mtimes to `SOURCE_DATE_EPOCH`. |
...
This release also patches six framework defects discovered
while bringing the migration to green CI. Three patches live
inside vigil's vendored copy and are tracked as drift against
upstream `pkg-framework` v1.2.1; the next `pkg-framework sync`
once upstream lands the fixes will resolve them all.

1. **`PKG_LAYOUT_CHECKS` mode format.** First attempt used
   octal-with-leading-zero (`0755`); framework's
   `_pkg_validate_stage` compares against `stat`'s unpadded
   output (`755`). All 14 entries corrected.
2. **`source` in container jobs.** The vendored
   `pkg-build.yml` uses `source pkg/project.sh` in seven
   places, several of them inside container jobs
   (`ubuntu:24.04`, `debian:12-slim`, `fedora:40`). Those
   containers' default shell is `/bin/sh` (dash), which has
   no `source` builtin. Added a workflow-level
   `defaults.run.shell: bash` so every `run:` step uses bash
   regardless of where it executes. *(framework FR)*
3. **Multi-line `PKG_DESCRIPTION` corrupts fpm args.**
   Framework's `_pkg_fpm_common_args` builds args via
   `printf '--description\n%s\n' "$PKG_DESCRIPTION"` and the
   caller reads the result line-by-line into an array. Each
   newline in the description ends up as a separate
   positional fpm arg; fpm interpreted the second paragraph
   as a path and refused to package. Documented as
   "newlines preserved" in framework docs but isn't. Vigil's
   `PKG_DESCRIPTION` collapsed to a single 387-char sentence.
   *(framework FR)*
4. **`_pkg_fpm_*` stdout pollution.** Framework v1.2.1's
   `_pkg_fpm_deb` and `_pkg_fpm_rpm` return the artifact
   path on stdout (`printf '%s' "$out_file"`) while
   simultaneously writing log chatter to stdout via
   `section` / `run` / `log`. `run_pkg_build` captures with
   `artifact="$(_pkg_fpm_deb)"`, so `$artifact` becomes the
   entire build log; `_pkg_validate_artifact` then tests
   `-s "<huge blob>"` and reports the file missing despite
   fpm just having written it. Patched the two functions to
   redirect `section`/`run`/`log` to stderr and leave only
   the final `printf` on stdout. *(framework FR, critical:
   the framework cannot end-to-end produce an artifact
   without this)*
5. **`_pkg_validate_artifact` SIGPIPE.** Pipes
   `dpkg-deb -c $artifact` / `rpm -qpl $artifact` directly
   into `head -20`. Vigil's deb has more than 20 entries; on
   the 20th line `head` closes the pipe, the producer takes
   SIGPIPE, and under `set -euo pipefail` the whole script
   dies even though the artifact is fine. Patched to capture
   the listing into a variable first (`$()` drains the
   producer to EOF), then `head -20`. *(framework FR)*
6. **Slim-image doc and man exclusions silently drop
   packaged content at install-test time.** `debian:12-slim`
   and `ubuntu:24.04` Docker images ship with
   `/etc/dpkg/dpkg.cfg.d/excludes` containing
   `path-exclude /usr/share/{doc,man,groff,info,lintian}/*`;
   `fedora:40` minimal sets `tsflags=nodocs` in
   `/etc/dnf/dnf.conf`. dpkg / dnf honor these excludes at
   install time, so the layout-check inside the test
   container saw README.md, changelog.gz, copyright, and
   every man page as "missing" with non-deterministic
   variation across the matrix. The artifacts themselves
   contain everything; the test container's view didn't.
   Patched install-test step to clear both before
   `apt-get install` / `dnf install`. *(framework FR)*

### Notes

- **Artifact filenames change.** Pre-framework:
  `vigil_<v>_amd64-<distro>.deb` with the distro embedded so
  noble/jammy/bookworm could coexist in one OUTDIR.
  Framework default: `vigil-baseline_<v>_amd64.deb` (no
  distro suffix). Per-distro uniqueness now relies on each
  matrix entry writing to its own OUTDIR. rpm equivalent:
  `vigil-baseline-<v>-1.x86_64.rpm`. Downstream tooling
  (`lousclues-pkg` orchestrator, release-page consumers)
  needs to expect the new names.
- **Cargo build is no longer hermetic.** Framework's
  `_pkg_build_binaries` runs `cargo build --release --locked`.
  The pre-framework build did `cargo fetch --locked` then
  `cargo build --frozen --offline`, which provably refused
  network during the compile step. `--locked` keeps the
  lockfile pinned but allows network. Tracked as a framework
  FR to add a hermetic-build knob (e.g. `PKG_CARGO_OFFLINE=1`).
- **Three drift markers reported by `pkg-framework verify`.**
  `.github/workflows/pkg-build.yml` (workflow shell + slim
  install excludes) and `pkg/lib/framework.sh` (stdout
  pollution + SIGPIPE) are intentionally patched ahead of
  upstream. The next `pkg-framework sync` after
  `lousclues-labs/pkg-integration` ships a release with the
  fixes will resolve all three drifts simultaneously.

### Framework follow-ups filed against `lousclues-labs/pkg-integration`

1. CLI symlink resolution: `$BASH_SOURCE[0]` doesn't follow
   the `~/.local/bin/pkg-framework` symlink so `version` /
   `doctor` can't find `VERSION` without an explicit
   `PKG_FRAMEWORK_HOME` override.
2. `_pkg_emit_debian_copyright` emits a truncated header and
   points the license body at `changelog.gz` (vigil's
   `project_stage_extra` overrides locally).
3. Hermetic build knob (item from Notes above).
4. Per-distro artifact filename suffix
   (`PKG_DEB_DIST_SUFFIX` / `PKG_RPM_DIST_TAG`) so multiple
   distros can coexist in one OUTDIR.
5. Workflow `source` in container jobs needs `shell: bash`.
6. `_pkg_fpm_common_args` cannot transport multi-line
   fields.
7. `_pkg_fpm_*` return-value-on-stdout collides with stdout
   logging.
8. `_pkg_validate_artifact` SIGPIPEs on packages with more
   than 20 entries.
9. Install-test step doesn't clear slim-image doc/man
   excludes.
10. `layout-check.sh` `grep -E '\\/...'` emits cosmetic
    "stray \ before /" warnings on modern GNU grep
    (fedora-40).

Items 5, 7, 8, 9 are live-patched in vigil's vendored copy.
Items 6 is fixed via single-line `PKG_DESCRIPTION` in
`pkg/project.sh`. Item 2 is fixed via
`project_stage_extra` override. The rest are open.

The framework would benefit from an end-to-end smoke test
that builds a placeholder package and asserts
`dpkg-deb -I`, `dpkg-deb -c`, `rpm -qpi`, `rpm -qpl` all
succeed. One such test would have caught items 7, 8, 9
simultaneously.
