...
When HMAC signing is disabled, a prominent warning is displayed explaining the distinction between chain integrity (verifiable — each entry links to its predecessor) and chain authenticity (not verifiable without HMAC — an attacker with write access to `audit.db` could forge a self-consistent chain). See [SECURITY.md](../SECURITY.md) for details.
