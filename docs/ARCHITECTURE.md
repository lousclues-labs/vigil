...
Vigil Baseline has one job. Detect filesystem boundary changes and record them.
...
- `src/control.rs` defines `ControlHandler`, a struct holding metrics, state, reload flag, scan trigger, DB path, HMAC key, auth flag, and a `shared_baseline_identity` reference for TOCTOU coordination during baseline refresh. Methods: `handle_connection` (auth-or-read, dispatch, write), `authenticate_and_read` (challenge-response), `read_request`, `write_response`, `dispatch`. The `handle_baseline_refresh_streaming` method signals `SharedBaselineIdentity::expect_baseline_replacement()` immediately before `fs::rename()` so the guardian thread accepts the inode change. When HMAC signing is enabled, connections are authenticated via nonce-based challenge-response. All `reload` and `scan` commands are logged with peer credentials.
- `src/coordinator.rs` defines `CoordinatorConfig` (spawn arguments), `SharedBaselineIdentity` (thread-safe TOCTOU coordination), and `Coordinator` (runtime state), then launches two loops: `vigil-guardian` at 1s cadence and `vigil-maintenance` at 60s cadence. Guardian handles watchdog heartbeats, fast backpressure checks, baseline-identity guardrails, and writes `guardian.json` (1Hz state snapshot). `SharedBaselineIdentity` wraps the baseline DB's inode/device in atomics alongside a monotonic-time replacement deadline (nanos-since-startup, wall-clock-independent); the control socket handler calls `expect_baseline_replacement(deadline)` before a refresh rename, and the guardian checks the deadline before degrading -- authorized replacements accept the new inode, unauthorized replacements degrade as before. The 30-second `BASELINE_REPLACEMENT_WINDOW` is fail-closed: expiry re-engages normal tamper detection. Maintenance writes `state.json` (60s cadence, slow-changing fields including drift velocity), handles reload application, DB/WAL identity checks, mount-evasion scans, clock anomaly checks (configurable via `security.clock_skew_threshold_seconds`), audit rotation with retention-skip tracking, **audit retention sweep**, event-drop checks, WAL checkpointing, maintenance timeout enforcement, and drift-velocity metric computation. `atomic_write()` uses `tempfile::NamedTempFile` for unique temp filenames per call, preventing collisions between concurrent writers. The retention sweep runs once per `audit.retention_check_interval` (default 24h), replaces old entries with a cryptographic `AuditCheckpoint` record, and deletes the originals in a single atomic transaction. See `docs/RETENTION.md`. DB connections remain `Option<Connection>` and are dropped on `*_db_replaced` / `wal_file_replaced` degraded transitions (VIGIL-VULN-071) so operators must explicitly recover.
- `src/worker.rs` defines `WorkerSpawnArgs` (spawn arguments) and `WorkerContext` (per-worker state: connection, config, watch index, metrics, filter, LRU cache, generation tracker, pre-computed self-protection paths). Key methods: `evaluate` (cache lookup + baseline + snapshot + diff + severity from watch group), `process_safe` (catch_unwind wrapper), `drain_debounced` (debounced re-check). Self-protection paths (config files + HMAC key) are computed once at startup and shared across all workers as `Arc<HashSet<PathBuf>>`; the per-event check is a single `HashSet::contains` lookup.
...
- `src/scan_scheduler.rs` parses cron strings with `croner` and executes scheduled scans. Receives `baseline_generation: Arc<AtomicU64>` at spawn and reopens its baseline database connection when the generation increments after a baseline refresh, preventing scans from comparing the filesystem against a stale pre-refresh baseline.
...
Vigil Baseline compares current state to baseline with a file-descriptor-first pipeline.
...
Vigil Baseline uses two SQLite files in practice.
...
| Guardian snapshot | `/run/vigil/guardian.json` |
