...
- Package-manager pre-hooks take a pre-transaction seal (`vigil maintenance enter --seal`): the watched set is scanned and every deviation recorded into the tamper-evident chain with a timestamp preceding the transaction, so drift that predates an update is distinguishable from drift the update introduced. Seal records carry no package attribution and therefore cannot be suppressed by the maintenance window they precede. A seal whose scan fails reports the failure and never reports a clean system. Limitation: the seal proves what the filesystem looked like at that instant, not that nothing changed between the seal and the first package write.
- A maintenance window is force-closed once it outlives `maintenance.max_window_seconds`, and that closure is durable: the on-disk breadcrumb is removed and a daemon start refuses to resume a breadcrumb older than the cap. A window suppresses every package-owned change at every severity, so one that never closed would be a silent hole in coverage that reopened on every boot.
- Baseline refresh verifies every changed file that a package owns against the digest that package recorded (`dpkg --verify`, `rpm -V`, `pacman -Qkk`). Package *ownership* of a path is never treated as evidence that the package wrote the bytes now in it. A file whose content contradicts its package is recorded as a `Severity::Critical` deviation and reported to the operator; it is never absorbed as a routine package update. A verification that could not run yields `Unknown`, never `Verified`, so a failed or unavailable verifier can never launder a change. Limitation: dpkg records MD5 digests, so this check inherits MD5's second-preimage resistance for Debian-family systems; vigil's own baseline remains BLAKE3 and the audit record is written regardless of the verdict.
