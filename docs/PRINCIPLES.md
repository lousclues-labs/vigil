...
Vigil exists because of one truth:

> **No one tiptoes through your system without leaving footprints behind.**

Vigil is the gatekeeper that refuses to let anything cross the threshold
...
Vigil is a watchman. It watches, compares, and reports.
...
When something changes that shouldn't have, Vigil tells you — immediately,
...
Vigil emits nothing during normal operation. No informational noise. No
...
compromised or Vigil is poorly configured. Fix the configuration before
you investigate the system.

**The test:** Run Vigil for seven days on a clean Arch install with normal
...
Vigil does not guess. It does not infer. It does not predict. It does not
...
the same baseline entry, does Vigil always produce the same result? If yes,
...
Vigil compares. It does not judge.
...
Every detection in Vigil can be expressed as a simple conditional statement
...
**The test:** Disconnect the network cable. Does Vigil work? If yes, it passes.

## VII. Boundaries, Not Intelligence

Vigil enforces the state boundary: *has anything changed that shouldn't have?*
...
## VIII. Vigil Stands Alone

Vigil is independently useful. It works without any other tool installed.
It can be built, tested, and deployed in isolation.

No tool imports Vigil. Vigil imports no tool. No tool reads Vigil's database.
Vigil reads no tool's database. If every other security tool on the system
vanishes, Vigil still compiles, runs, and fulfills its core function.

A user who runs `vigil init && vigil watch` with zero other tools installed
is meaningfully more secure than they were before.

**The test:** Uninstall everything else. Does Vigil still work? If not,
there's a dependency that shouldn't exist.

## IX. No Configuration Required for Correct Operation

Vigil ships with sensible defaults that provide real security out of the box.
...
When Vigil encounters an error, two things happen:
...
**Fail loud:** The failure is visible. If fanotify is unavailable and Vigil
...
**The test:** Remove `CAP_SYS_ADMIN`. Start Vigil. Does it clearly communicate
...
Vigil is small. It hashes files, compares metadata, and writes alerts.
That's it. When deciding whether to add a feature, ask:

- Does this make Vigil better at watching the filesystem?
- Or does this make Vigil do a second job?

If it's a second job, it's a second tool. Or it doesn't exist.

**The test:** Can you explain Vigil's entire architecture in a single ASCII
...
Vigil operates entirely locally. No telemetry. No update checks. No cloud
...
**The test:** Disconnect the network cable. Does Vigil work identically?

## XV. The User Is the Operator

Vigil is designed for someone who understands their system, reads alerts
...
Vigil provides information, not opinions. The operator decides.
...
> **"Does this make Vigil quieter or noisier?"**

If it makes Vigil quieter by removing false positives, eliminating ambiguity,
or suppressing redundant information — it belongs.

If it makes Vigil noisier by adding uncertain signals, soft thresholds,
...
**Vigil promises: *You know when something changes that shouldn't have.***

When Vigil is silent, your boundaries are intact — because you tested the
conditions under which it alerts, and none of those conditions are present.

When Vigil speaks, something real has happened — because every detection is
