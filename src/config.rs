...
/// Compare two configs and return human-readable descriptions of what changed.
pub fn diff_config(old: &Config, new: &Config) -> Vec<String> {
    ...
    if old.alerts.desktop_notifications != new.alerts.desktop_notifications {
        diffs.push(format!(
            "desktop_notifications changed: {} → {}",
            old.alerts.desktop_notifications, new.alerts.desktop_notifications
        ));
    }
    ...
    for (group_name, new_group) in &new.watch {
        if let Some(old_group) = old.watch.get(group_name) {
            ...
            let old_paths: std::collections::HashSet<&String> = old_group.paths.iter().collect();
            let new_paths: std::collections::HashSet<&String> = new_group.paths.iter().collect();
    ...
    // Exclusion patterns
    let old_excl: std::collections::HashSet<&String> = old.exclusions.patterns.iter().collect();
    let new_excl: std::collections::HashSet<&String> = new.exclusions.patterns.iter().collect();
