...
//! threads, and owns `DaemonRuntime` for coordinated shutdown.
...
use crate::{control, coordinator, monitor, scan_scheduler, scanner, worker};
    ...
    fn start(daemon: &Daemon) -> Result<Self> {
        ...
        let control_handle = if !cfg.daemon.control_socket.as_os_str().is_empty() {
            ...
            let handler = control::ControlHandler {
                ...
                shared_baseline_identity: shared_baseline_identity.clone(),
                baseline_generation: baseline_generation.clone(),
                expectation_registry: Some(Arc::new(coordinator::ExpectationRegistry::new())),
            };
            Some(control::spawn(
