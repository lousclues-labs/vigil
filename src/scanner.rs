...
/// Callback interface for baseline-build progress reporting.
pub trait BaselineProgress {
    /// Called after a pre-walk to count total files. Return false to skip the pre-walk.
    fn wants_total(&self) -> bool {
        false
    }
    fn pre_walk_complete(&self, _total: u64) {}
    fn entry_processed(&self, _processed: u64, _total: u64) {}
    fn phase(&self, _name: &str) {}
}

/// No-op implementation for callers that don't need progress.
pub struct SilentProgress;
impl BaselineProgress for SilentProgress {}

/// Build a full initial baseline by walking all configured watch paths.
pub fn build_initial_baseline(conn: &Connection, config: &Config) -> Result<BaselineInitResult> {
    build_initial_baseline_impl(conn, config, &SilentProgress)
}
...
where
    F: Fn(u64, u64, &str),
{
    struct ClosureProgress<'a, F>(&'a F);
    impl<F: Fn(u64, u64, &str)> BaselineProgress for ClosureProgress<'_, F> {
        fn wants_total(&self) -> bool {
            true
        }
        fn pre_walk_complete(&self, total: u64) {
            (self.0)(0, total, "scanning");
        }
        fn entry_processed(&self, processed: u64, total: u64) {
            (self.0)(processed, total, "scanning");
        }
        fn phase(&self, name: &str) {
            (self.0)(0, 0, name);
        }
    }
    build_initial_baseline_impl(conn, config, &ClosureProgress(progress))
}

fn build_initial_baseline_impl<P: BaselineProgress>(
    conn: &Connection,
    config: &Config,
    progress: &P,
) -> Result<BaselineInitResult> {
    ...
    // Pre-walk to count files if progress tracking is requested
    let mut estimated_total = 0u64;
    if progress.wants_total() {
        ...
        progress.pre_walk_complete(estimated_total);
    }
    ...
    let package_cache = if !skip_package_owner {
        ...
        let cache = crate::package::build_package_cache(&config.package_manager);
        if progress.wants_total() {
