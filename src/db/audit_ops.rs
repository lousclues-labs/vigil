...
pub fn insert_receipt_entry(
    ...
    let timestamp = Utc::now().timestamp();
    let path = AuditEventPath::CheckCompleted.as_str();
    let severity = "info";
...
pub fn insert_self_check_entry(
    ...
    let timestamp = Utc::now().timestamp();
    let path = AuditEventPath::SelfCheck.as_str();
...
pub fn insert_test_alert_entry(
    ...
    let timestamp = Utc::now().timestamp();
    let path = AuditEventPath::TestAlert.as_str();
...
/// Operator context for any state-modifying CLI action.
/// Built once per command invocation; included in the audit record's payload.
...
/// Record an operator-initiated action in the audit chain.
///
/// Single source of truth for chain-extending operator actions.
/// All `vigil ack`, `vigil hooks {disable,enable}`, and future
/// operator commands route through this function.
///
/// Returns the audit record's sequence number on success.
pub fn record_operator_action(
    config: &crate::config::Config,
    path: AuditEventPath,
    payload: serde_json::Value,
) -> Result<i64> {
    let conn = crate::db::open_audit_db(config)?;
    let previous_chain_hash = get_last_chain_hash(&conn)?.unwrap_or_else(|| {
    ...
    let operator = OperatorContext::from_current_process();
    let full_payload = serde_json::json!({
        "action": payload,
        "operator": operator,
    });
    let payload_json = serde_json::to_string(&full_payload)?;
...
// Re-export retention/checkpoint types for backward compatibility.
pub use super::audit_retention::{
    build_checkpoint_hmac_data, checkpoint_covered_entries, compute_checkpoint_chain_hash,
    count_detection_entries, count_entries, db_file_size, delete_detection_range,
    get_previous_chain_hash, identify_prune_range, insert_acknowledgment_entry, insert_checkpoint,
    insert_doctor_event_entry, insert_hooks_operation_entry, list_checkpoints, list_segments,
    oldest_checkpoint_timestamp, oldest_detection_timestamp, read_detection_range, AuditCheckpoint,
    AuditSegment,
};
