...
fn audit_chain_break_detected_after_deletion() {
    ...
    let h1 = vigil::db::audit_ops::insert_audit_entry(&conn, &c1, false, false, None, &genesis)
        .unwrap();

    let c2 = sample_change("/tmp/b");
    let _h2 = vigil::db::audit_ops::insert_audit_entry(&conn, &c2, false, false, None, &h1)
        .unwrap();
    ...
    conn.execute("DELETE FROM audit_log WHERE id = 1", []).unwrap();
