...
fn run_wal_concurrency(seed: u64) {
    ...
    artifacts.record(0, format!("Starting {} threads, {} ops each", num_threads, ops_per_thread));
    ...
    for e in &entries {
        engine.check(
            ...
            !e.fully_consumed(),
            format!("iter_unconsumed returned fully consumed entry seq={}", e.sequence),
        );
    }
    ...
    engine.check(
        ...
        format!("pending_count {} != iter_unconsumed len {}", pending, manual_pending),
    );
